...
public:
    ...
    vector<int> dfs(int u,int parrent,vector<vector<int>> & adj, vector<int> & ans, string & labels){   //root, adj, ans, labels
        vector<int> currentFreq(26, 0);
        for(int v : adj[u]){
            if(v == parrent) continue;
            vector<int> freqChild = dfs(v, u, adj, ans, labels);
